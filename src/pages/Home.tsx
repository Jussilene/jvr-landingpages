...
const audiences = [
  ['Autônomos', 'Centralize sua apresentação, seus serviços e seu contato em um único link profissional.'],
  ['Pequenos negócios', 'Mostre seus diferenciais e facilite pedidos de orçamento diretamente pelo WhatsApp.'],
  ['Especialistas', 'Apresente sua experiência, seu método e seus serviços com clareza e credibilidade.'],
  ['Prestadores de serviço', 'Divulgue serviços, regiões atendidas e formas de contato de maneira simples e profissional.']
]

const processSteps = [
  ['Escolha o template', 'Escolha o estilo que mais combina com o seu negócio e com a imagem que deseja transmitir.'],
  ['Solicite o orçamento', 'Fale pelo WhatsApp para definirmos o plano, o escopo, o prazo e possíveis adicionais.'],
  ['Envie todo o conteúdo', 'Envie textos, imagens, vídeos, áudios e contatos. O prazo começa após esta etapa.'],
  ['A página é personalizada', 'Aplicamos suas cores, textos, imagens, informações e identidade ao modelo escolhido.'],
  ['Aprove e vá para o ar', 'Você revisa o resultado, solicita os ajustes incluídos e aprova a publicação da página.']
]
    ...
    <main ref={mainRef}>
      <section className="hero" onPointerMove={moveGlow}>
        <div className="hero-inner">
          <div className="hero-copy"><span className="eyebrow"><Sparkles size={16}/> Presença digital sem complicação</span><h1>Sua landing page profissional, <em>bonita e pronta para vender.</em></h1><p>Escolha um dos nossos modelos, envie seus textos, imagens, vídeos ou áudios e receba sua página personalizada e publicada com rapidez.</p><div className="hero-actions"><a className="button" href={whatsappUrl()} target="_blank" rel="noopener noreferrer"><WhatsAppIcon/> Solicitar orçamento</a><a className="button secondary" href="#modelos">Visualizar templates <ArrowRight size={18}/></a></div>
      ...
      <section className="templates-section" id="modelos"><div className="container section-heading"><span className="eyebrow">Escolha seu ponto de partida</span><h2>Modelos com personalidade.<br/>Personalizados para a sua marca.</h2><p>Cada demonstração é uma página funcional completa. Abra, explore e imagine seu conteúdo ali.</p></div>
        <div className="filters" aria-label="Filtrar modelos">{categories.map(c=><button key={c} className={filter===c?'active':''} aria-pressed={filter===c} onClick={()=>setFilter(c)}>{c}</button>)}</div>
        <div className="container template-grid">{visible.map(t=><article data-motion className={`template-card card-${t.palette}`} key={t.slug}><Link className="preview" to={`/templates/${t.slug}`} aria-label={`Visualizar template ${t.name}`}><span className="preview-browser"><i/><i/><i/><b>jvr.page/{t.slug}</b></span><img src={`${base}previews/${t.slug}.webp`} alt={`Prévia real do template ${t.name}`} loading="lazy" /><span className="open-hint">Abrir projeto <ArrowRight size={15}/></span></Link><div className="template-info"><div className="template-meta"><span className="tag">{t.category}</span><strong>A partir de R$ 500</strong></div><h3>{t.name}</h3><p>{t.niche} · {t.style}</p><div className="template-features"><span>Responsivo</span><span>WhatsApp</span><span>SEO básico</span></div><div className="card-actions"><Link to={`/templates/${t.slug}`}>Ver demonstração</Link><a href={whatsappUrl(`Olá! Gostei do template ${t.name} e gostaria de contratar esse modelo.`)} target="_blank" rel="noopener noreferrer" aria-label={`Solicitar orçamento do template ${t.name}`}><WhatsAppIcon size={15}/> Orçamento</a></div></div></article>)}</div>
      </section>

      <section className="light-section" id="planos"><div className="container section-heading dark-heading"><span className="eyebrow dark">Planos e valores</span><h2>Simples de entender.<br/>Fácil de começar.</h2><p>*O prazo começa após o recebimento de todo o material solicitado.</p></div><div className="container pricing-grid">{buildPlans.map(p=><article className={p.featured?'price-card featured':'price-card'} key={p.name}>{p.featured&&<span className="popular">Mais escolhido</span>}<h3>{p.name}</h3><strong className="price">{p.price}</strong><p>{p.description}</p><ul>{p.items.map(x=><li key={x}><Check/>{x}</li>)}</ul><a className="button" href={whatsappUrl(`Olá! Gostaria de contratar o plano ${p.name}.`)} target="_blank" rel="noopener noreferrer"><WhatsAppIcon size={18}/> Escolher este plano</a></article>)}</div><div className="container scope-note"><ShieldCheck/><p><strong>Escopo objetivo:</strong> sistemas, lojas virtuais, login, painéis, automações e integrações avançadas precisam de orçamento separado.</p></div></section>
      ...
      <section className="process light-section" id="como-funciona"><div className="container section-heading dark-heading"><span className="eyebrow dark">Como funciona</span><h2>Da escolha à publicação,<br/>um caminho bem simples.</h2></div><div className="container steps">{processSteps.map(([title,description],i)=><article key={title}><span>{i+1}</span><h3>{title}</h3><p>{description}</p></article>)}</div></section>
      ...
      <section className="final-cta"><div><span className="eyebrow">Seu próximo passo</span><h2>Vamos colocar seu trabalho no ar?</h2><p>Escolha seu modelo preferido e fale pelo WhatsApp para receber um orçamento.</p><a className="button light-button" href={whatsappUrl()} target="_blank" rel="noopener noreferrer"><WhatsAppIcon/> Quero minha landing page <ArrowRight/></a></div></section>
